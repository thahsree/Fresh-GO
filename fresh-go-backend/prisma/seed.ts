...
async function main() {
  ...
  for (const item of productsToSeed) {
    ...
    const activeBatchCount = await prisma.inventoryBatch.count({
      where: { productId: product.id, isActive: true, freshnessStatus: FreshnessStatus.FRESH },
    });
  ...
  if (seerProduct && snapperProduct && partnerProfile) {
    const activeOrder = await prisma.order.upsert({
      ...
      create: {
        ...
        statusEvents: {
          create: [
            { toStatus: OrderStatus.PLACED, createdAt: new Date(Date.now() - 35 * 60 * 1000) },
            { toStatus: OrderStatus.CUTTING_PREPARING, createdAt: new Date(Date.now() - 25 * 60 * 1000) },
            { toStatus: OrderStatus.PACKED, createdAt: new Date(Date.now() - 18 * 60 * 1000) },
            { toStatus: OrderStatus.OUT_FOR_DELIVERY, createdAt: new Date(Date.now() - 12 * 60 * 1000) },
          ],
        },
      },
    });
  ...
  if (prawnsProduct && salmonProduct && partnerProfile) {
    const pastOrder = await prisma.order.upsert({
      ...
      create: {
        ...
        statusEvents: {
          create: [
            { toStatus: OrderStatus.PLACED, createdAt: new Date(Date.now() - 26 * 3600 * 1000) },
            { toStatus: OrderStatus.DELIVERED, createdAt: new Date(Date.now() - 24.5 * 3600 * 1000) },
          ],
        },
      },
    });
